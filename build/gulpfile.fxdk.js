...
function defineTasks(options = {}) {
	...
	function packageWebTask(sourceFolderName, destinationFolderName) {
		...
		return () => {
			...
			const productJsonStream = gulp.src(['product.json'], { base: './resources/fxdk' })
			...
			const indexStream = gulp.src([`out-build/vs/fxdk/browser/workbench/workbench.html`], { base: `out-build/vs/fxdk/browser/workbench` })
				.pipe(rename('index.html'))
				.pipe(replace('static/', ''));

			const manifest = gulp.src(`resources/fxdk/manifest.json`, { base: 'resources/fxdk' })
				.pipe(json({
