...
import { Query, VscodeOptions, WorkbenchOptions } from 'vs/fxdk/common/ipc';
...
import { getLocaleFromConfig, getNlsConfiguration } from 'vs/fxdk/node/nls';
...
import { FxDKEnvironmentService, IFxDKEnvironmentService } from 'vs/fxdk/node/impl/fxdkEnvironmentService';
...
export class Vscode {
	...
	public async initialize(options: VscodeOptions): Promise<WorkbenchOptions> {
		...
		const environment = this.services.get(IEnvironmentService) as IFxDKEnvironmentService;
		...
		await new Promise((resolve) => {
			...
			instantiationService.invokeFunction((accessor) => {
				...
				const ptyHostService = new PtyHostService({
					...
					shortGraceTime: 6000,
					scrollback: 80,
					useExperimentalSerialization: false,
