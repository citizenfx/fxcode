import { field, Logger, logger } from '../logger';
...
import { getNlsConfiguration } from 'vs/fxdk/node/nls';
	...
	private async spawn(buffer: VSBuffer, inflateBytes: Uint8Array | undefined): Promise<cp.ChildProcess> {
		...
		const proc = cp.fork(
			...
			[
				...
				`--uriTransformerPath=${FileAccess.asFileUri('serverUriTransformer.js', require).fsPath}`
			],
			{
				env: {
					...process.env,
