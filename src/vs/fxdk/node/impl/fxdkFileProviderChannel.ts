...
import { FileType, IFileDeleteOptions, IFileOpenOptions, IFileOverwriteOptions, IFileReadStreamOptions, IFileWriteOptions, IStat, IWatchOptions } from 'vs/platform/files/common/files';
...
export class FxDKFileProviderChannel implements IServerChannel<RemoteAgentConnectionContext>, IDisposable {
	...
	private filechange(context: RemoteAgentConnectionContext, session: string): Event<IFileChangeDto[]> {
		const emitter = new Emitter<IFileChangeDto[]>({
			onFirstListenerAdd: () => {
				...
				provider.onDidWatchError((event) => this.logService.error(event));
			},
	...
	private readFileStream(resource: UriComponents, opts: IFileReadStreamOptions): Event<ReadableStreamEventPayload<VSBuffer>> {
	...
	private async open(resource: UriComponents, opts: IFileOpenOptions): Promise<number> {
	...
	private writeFile(resource: UriComponents, buffer: VSBuffer, opts: IFileWriteOptions): Promise<void> {
		return this.provider.writeFile(this.transform(resource), buffer.buffer, opts);
	}

	private async delete(resource: UriComponents, opts: IFileDeleteOptions): Promise<void> {
	...
	private async rename(resource: UriComponents, target: UriComponents, opts: IFileOverwriteOptions): Promise<void> {
		return this.provider.rename(this.transform(resource), URI.from(target), opts);
	}

	private copy(resource: UriComponents, target: UriComponents, opts: IFileOverwriteOptions): Promise<void> {
	...
	private transform(resource: UriComponents): URI {
		...
		} else if (resource.scheme === Schemas.vscodeUserData) {
