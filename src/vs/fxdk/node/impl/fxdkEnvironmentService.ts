...
export class FxDKEnvironmentService extends AbstractNativeEnvironmentService implements IFxDKEnvironmentService {
	...
	constructor(args: NativeParsedArgs, productService: IProductService) {
		super(args, {
			homeDir: path.join(cfxLocalAppData, 'sdk-personality-code-home'),
			userDataDir: path.join(cfxLocalAppData, 'sdk-personality-code-user-data'),
			tmpDir: os.tmpdir(),
		}, productService);
	...
	get extraBuiltinExtensionPaths(): string[] {
		return (this.__args['extra-builtin-extensions-dir'] || []).map((p: any) => resolve(p));
	}

	@memoize
	override get extensionsPath(): string {
		return path.join(cfxLocalAppData, 'sdk-personality-code-exts');
	}
}
